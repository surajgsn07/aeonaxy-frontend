...
const Select = () => {
  ...
  const setData = async()=>{
    ...
    try {
      const response = await axios.post("https://aenaxy-backend.vercel.app/api/v1/user/setPurpose" ,data,{
        withCredentials:true
      });
      console.log(response)
      if(response){
        try {
          const response = await axios.post("https://aenaxy-backend.vercel.app/api/v1/user/sendEmail" ,data,{
            withCredentials:true
          });
          if(response){
            navigate("/email")
